...
# ==============================================================================
# Main Split Conformal Prediction Function (MFTS)
# ==============================================================================
...
run_mfts_conformal_experiment <- function(way_ncomp, level_sig) {
  err_F <- err_M <- array(NA, dim = c(16, 3, 47))
  
  for (ij in 1:47) {
    for (iw in 1:16) {
      dum <- interval_fore_subnational_cdf_MFTS_conformal(
...
res_EVR_080 <- run_mfts_conformal_experiment(way_ncomp = "EVR",     level_sig = 0.80)
res_K6_080  <- run_mfts_conformal_experiment(way_ncomp = "provide", level_sig = 0.80)

# Level of significance = 0.95
res_EVR_095 <- run_mfts_conformal_experiment(way_ncomp = "EVR",     level_sig = 0.95)
res_K6_095  <- run_mfts_conformal_experiment(way_ncomp = "provide", level_sig = 0.95)
...
save_mfts_results <- function(res, method_label, suffix) {
  # Arrays
  saveRDS(res$err_F, file.path(dir.results, paste0("MFTS_int_fore_subnational_err_F_", method_label, "_conformal", suffix, ".rds")))
  saveRDS(res$err_M, file.path(dir.results, paste0("MFTS_int_fore_subnational_err_M_", method_label, "_conformal", suffix, ".rds")))
  
  # Means
  saveRDS(res$mean_F, file.path(dir.results, paste0("MFTS_int_fore_subnational_err_F_", method_label, "_conformal", suffix, "_mean.rds")))
  saveRDS(res$mean_M, file.path(dir.results, paste0("MFTS_int_fore_subnational_err_M_", method_label, "_conformal", suffix, "_mean.rds")))
}

# --- level_sig = 0.80 ---
save_mfts_results(res_EVR_080, "EVR", "")
save_mfts_results(res_K6_080,  "K6",  "")

# --- level_sig = 0.95 ---
save_mfts_results(res_EVR_095, "EVR", "_alpha_0.95")
save_mfts_results(res_K6_095,  "K6",  "_alpha_0.95")
...
arrays_list_shiny <- list(
  MFTS_female_EVR_80 = res_EVR_080$err_F,
  MFTS_male_EVR_80   = res_EVR_080$err_M,
  MFTS_female_K_80   = res_K6_080$err_F,
  MFTS_male_K_80     = res_K6_080$err_M,
  
  MFTS_female_EVR_95 = res_EVR_095$err_F,
  MFTS_male_EVR_95   = res_EVR_095$err_M,
  MFTS_female_K_95   = res_K6_095$err_F,
  MFTS_male_K_95     = res_K6_095$err_M
)
