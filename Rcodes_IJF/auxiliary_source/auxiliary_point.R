...
fore_national_cdf <- function(data_set, ncomp_method, fh, fmethod)
{
    ...
    data_cumsum_logit_fore <- suppressWarnings(
      forecast(
        ftsm(fts(ages[1:110], t(data_cumsum_logit)), order = ncomp),
        h = fh,
        method = fmethod
      )
    )
...
# horizon: forecast horizon 1 to 17
...
point_fore_national_cdf <- function(fdata, sex, method_ncomp, horizon, fore_method, uni_fore_method,
                                    CLR_ncomp_selection)
{
    fore_val = matrix(NA, ncol(fdata), (18 - horizon))
    if(fore_method == "CDF")
    {
    ...
    else if(fore_method == "CLR")
    {
        for(ij in 1:(18 - horizon))
        {
            fore_val[,ij] <- as.numeric(clr_fun(fdata = fdata[1:(32 + ij),], ncomp_selection = CLR_ncomp_selection,
    ...
    holdout_val_dum = t(matrix(fdata[(32 + horizon):49,], length((32 + horizon):49), ncol(fdata)))
    ...
    KL_div_val = JS_div_val = L1_dist = L2_dist = vector("numeric", (18 - horizon))
    for(ij in 1:(18 - horizon))
    {
        # symmetric KL dist
...
MFTS_model <- function(data_input, ncomp_method, fh, fore_method)
{
    ...
    fore_ftsm <- suppressWarnings(
      forecast(
        ftsm(fts(1:nrow(comb_object), comb_object), order = ncomp),
        h = fh,
        method = fore_method
      )
    )
...
point_fore_national_cdf_MFTS <- function(fdata_F, fdata_M, fore_method, horizon, way_ncomp, uni_fore_method)
{
  ...
  holdout_val_dum_F = t(matrix(fdata_F[(32 + horizon):49,], length((32 + horizon):49), ncol(fdata_F)))
  holdout_val_dum_M = t(matrix(fdata_M[(32 + horizon):49,], length((32 + horizon):49), ncol(fdata_F)))
  ...
  KL_div_val_F = JS_div_val_F = L1_dist_F = L2_dist_F =
    KL_div_val_M = JS_div_val_M = L1_dist_M = L2_dist_M = vector("numeric", (18 - horizon))
  for(ij in 1:(18 - horizon))
  {
  ...
  return(list(
    forecast_pdf_F = forecast_val_F, forecast_pdf_M = forecast_val_M,
...
point_fore_national_cdf_MLFTS <- function(fdata_F, fdata_M, fdata_common, fore_method, horizon, way_ncomp)
{
    ...
    colnames(forecast_val_F) = colnames(forecast_val_M) = 1:(18 - horizon)
                                             ...
                                             level_sig)
{
    ...
    holdout_validation_F = t(matrix(fdata_F[(17 + horizon):32,], length((17 + horizon):32), ncol(fdata_F)))
    holdout_validation_M = t(matrix(fdata_M[(17 + horizon):32,], length((17 + horizon):32), ncol(fdata_M)))
    ...
    forecast_test_F = forecast_test_F_lb = forecast_test_F_ub = matrix(NA, ncol(fdata_F), (18 - horizon))
    forecast_test_M = forecast_test_M_lb = forecast_test_M_ub = matrix(NA, ncol(fdata_M), (18 - horizon))
    if(fore_method == "CDF")
    {
        for(ij in 1:(18 - horizon))
        {
    ...
    holdout_val_F = t(matrix(fdata_F[(32 + horizon):49,], length((32 + horizon):49), ncol(fdata_F)))
    holdout_val_M = t(matrix(fdata_M[(32 + horizon):49,], length((32 + horizon):49), ncol(fdata_M)))
...
hdfpca_fun <- function(fdata_F, fdata_M, horizon, first_order, second_order, transformation)
{
  ...
  else if(transformation == "CDF")
  {
    ...
    {
      index_end = 49 - (18 - horizon) + ijk
      data_F = fdata_F[1:index_end, ] / 10^5
      data_M = fdata_M[1:index_end, ] / 10^5
  ...
  start_index <- 32 + horizon        
  end_index <- 49                    
  
  rows_to_extract <- start_index:end_index
  num_rows <- length(rows_to_extract)
  
  holdout_val_dum_F <- t(matrix(fdata_F[rows_to_extract, ], num_rows, ncol(fdata_F)))
  holdout_val_dum_M <- t(matrix(fdata_M[rows_to_extract, ], num_rows, ncol(fdata_F)))
...
hdfpca_fun_fore <- function(fdata_F, fdata_M, horizon, first_order, second_order, transformation)
{
  ...
  if(transformation == "direct")
  {
    for(ij in 1:(18 - horizon))
    {
      data_F = fdata_F[1:(nrow(fdata_F)-18+ij),]
      data_M = fdata_M[1:(nrow(fdata_F)-18+ij),]
      data_comb = list()
      data_comb[[1]] = t(data_F)
      data_comb[[2]] = t(data_M)
