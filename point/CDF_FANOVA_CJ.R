...
for (i in 1:n_states)
{
  ...
  # iwk is the expanding window. Therefore you iterate max_h times. Boot is a list of matrices 2*n_age by K=17,...,1.
  ...
  for (h in seq_along(Forecast_female_EVR)) {
    ...
    n_cols <- ncol(fore_val_EVR_1) # Number of columns (varies from 17 to 1)
...
##############################################################################
source("~/Library/CloudStorage/GoogleDrive-cristian.jimenezvaron@york.ac.uk/My Drive/HDFTS_CDF/HDFTS_CDF/code_submission/auxiliary_source/forecast_errors.R")
...
create_new_list <- function(A) {
  # Number of columns in the largest matrix (A[[1]]) is 17
  num_columns <- 17
  
  # Initialize an empty list for B
  B <- vector("list", num_columns)
  
  # Loop over each column index (1 to 17)
...
errors_KLD_female_EVR <- matrix(NA,17,47)
errors_KLD_male_EVR <- matrix(NA,17,47)

errors_JSD_female_EVR <- matrix(NA,17,47)
errors_JSD_male_EVR <- matrix(NA,17,47)

errors_KLD_female_K <- matrix(NA,17,47)
errors_KLD_male_K <- matrix(NA,17,47)

errors_JSD_female_K <- matrix(NA,17,47)
errors_JSD_male_K <- matrix(NA,17,47)
for (i in 1:n_states) {
  ...
  # This are the list of errors for each expanding window (EW). That is for EW=1, I compute the pairwise error between forecasts h=1,...,h=17
  # with holdout data for years 33 to 49.
  errors_female_EVR_list <- vector("list", 17)  # Each entry will store 2 errors for each horizon
  errors_male_EVR_list   <- vector("list", 17)
  errors_female_K_list <- vector("list", 17)  # Each entry will store 2 errors for each horizon
  errors_male_K_list   <- vector("list", 17)
  
  # Iterate through the Expanding windows (h = 1 to 17)
  ...
  # Now we need to aggregate errors over all h. That is we need to cbind for h=1 (in the 17 EW),h=2(in the 15 EW),...,h=17(in 1 EW). 
  
  # what this does is to get the h=1 from each expanding window (17 times). Then the h=2 (15 times)...h=17 (one time). Then create matrices with these errors. 
  ...
  errors_female_EVR_final <- matrix(NA, 2, 17)  # 2 error metrics × 17 forecast horizons
  errors_male_EVR_final   <- matrix(NA, 2, 17)
  
  errors_female_K_final <- matrix(NA, 2, 17)  # 2 error metrics × 17 forecast horizons
  errors_male_K_final   <- matrix(NA, 2, 17)
...
saveRDS(errors_KLD_female_EVR, file = paste0(dir.r, "errors_KLD_female_EVR.rds"))
saveRDS(errors_JSD_female_EVR, file = paste0(dir.r, "errors_JSD_female_EVR.rds"))
saveRDS(errors_KLD_male_EVR,   file = paste0(dir.r, "errors_KLD_male_EVR.rds"))
saveRDS(errors_JSD_male_EVR,   file = paste0(dir.r, "errors_JSD_male_EVR.rds"))

saveRDS(errors_KLD_female_K, file = paste0(dir.r, "errors_KLD_female_K.rds"))
saveRDS(errors_JSD_female_K, file = paste0(dir.r, "errors_JSD_female_K.rds"))
saveRDS(errors_KLD_male_K,   file = paste0(dir.r, "errors_KLD_male_K.rds"))
saveRDS(errors_JSD_male_K,   file = paste0(dir.r, "errors_JSD_male_K.rds"))

saveRDS(errors_pref_EVR, file = paste0(dir.r, "errors_pref_EVR_list.rds"))
saveRDS(errors_pref_K,   file = paste0(dir.r, "errors_pref_K_list.rds"))

saveRDS(errors_EVR, file = paste0(dir.r, "errors_EVR_matrix.rds"))
saveRDS(errors_K,   file = paste0(dir.r, "errors_K_matrix.rds"))

# Summary statistics
saveRDS(colMeans(errors_EVR), file = paste0(dir.r, "errors_EVR_colMeans.rds"))
saveRDS(apply(errors_EVR, 2, median), file = paste0(dir.r, "errors_EVR_median.rds"))

saveRDS(colMeans(errors_K), file = paste0(dir.r, "errors_K_colMeans.rds"))
saveRDS(apply(errors_K, 2, median), file = paste0(dir.r, "errors_K_median.rds"))
...
errors_KLD_female_EVR <- readRDS(paste0(dir.r1, "errors_KLD_female_EVR.rds"))
errors_JSD_female_EVR <- readRDS(paste0(dir.r1, "errors_JSD_female_EVR.rds"))
errors_KLD_male_EVR   <- readRDS(paste0(dir.r1, "errors_KLD_male_EVR.rds"))
errors_JSD_male_EVR   <- readRDS(paste0(dir.r1, "errors_JSD_male_EVR.rds"))

errors_KLD_female_K <- readRDS(paste0(dir.r1, "errors_KLD_female_K.rds"))
errors_JSD_female_K <- readRDS(paste0(dir.r1, "errors_JSD_female_K.rds"))
errors_KLD_male_K   <- readRDS(paste0(dir.r1, "errors_KLD_male_K.rds"))
errors_JSD_male_K   <- readRDS(paste0(dir.r1, "errors_JSD_male_K.rds"))
...
# Loop and save with FANOVA prefix
for(name in names(fanova_list)) {
  
  mat <- fanova_list[[name]]
  
  # Extract error metric, gender, and component from name
